...
function solveChallenge(path, method) {
    return new Promise((resolve, reject) => {
        ...
        console.log(`Solving challenge ${id} for ${path} (${method})`);
...
(async () => {
    try {
        ...
        let homepageData = await fetch(`https://${location.hostname}/`).then(
            (res) => res.text()
        );
            ...
            solverIframe.contentWindow.postMessage(
                {
                    ...
                    anims,
                    verificationCode: dom.querySelector(
